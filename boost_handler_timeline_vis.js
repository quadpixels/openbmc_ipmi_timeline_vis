// This script deals with Boost handlers
...
const FILENAME_BOOST_HANDLER = "/usr/local/google/home/suichen/Documents/semitruck_perf/boost_asio_trace/fdeo29_20200617_psusensor_boost_handlers";
...
function ParseBoostHandlerTimeline(content) {
  ...
  const lines = content.split("\n");
	...
	console.log("Boost handler log: " + parsed_entries.length + " entries" +
	            ", " + slots.length + " levels")
...
function Group_ASIO(preprocessed, group_by) {
  let grouped = { };
	...
	console.log("[Group_ASIO] |grouped|=" + grouped.length)
	return grouped
}
...
function GenerateTimeLine_ASIO(grouped) {
  ...
  // Time shift
	...
	console.log("[GenerateTimeLine_ASIO] |intervals|=" + intervals.length)
	console.log("[GenerateTimeLine_ASIO] |titles|=" + titles.length)

	boost_asio_handler_timeline_view.Intervals = intervals.slice()
	boost_asio_handler_timeline_view.Titles    = titles.slice()
}

function TestFileRead_BoostHandler() {
  let data = fs.readFileSync(FILENAME_BOOST_HANDLER, {encoding:"utf-8"});
