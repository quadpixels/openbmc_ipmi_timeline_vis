...
const FILENAME_TXT  = "nkwz3.txt"
const FILENAME_JSON = "nkwz3.json"

var g_temp0;

function TestFileRead() {
	let data = fs.readFileSync(FILENAME_TXT, { encoding:"utf-8" });
	{
  ...
  data = fs.readFileSync(FILENAME_JSON, { encoding:"utf-8" })
	{
		//  ret = JSON.parse(data)
		temp1 = []
		let lines = data.split("\n")
...
//	if (x.length > 1) ret = ret + BigInt(x[1])
	return ret
}
	  ...
	  let timestamp_end  = undefined
		...
		switch (ty) {
			case 4: { // signal
				...
				let entry = [ "sig", timestamp, serial, sender, destination, path,
											iface, member, timestamp_end, payload ];
			...
			case 1: { // method call
				...
				sender       = fixed_header[1][4][1];
				let entry = [ "mc", timestamp, serial, sender, destination, path,
											iface, member, timestamp_end, payload ];
				...
				} else if (iface == "xyz.openbmc_project.Ipmi.Server" &&
					         member == "execute") {

					let ipmi_entry = {
						netfn: payload[0], lun: payload[1], cmd: payload[2],
            request: payload[3],
...
//		console.log(i + ", " + ty + ", " + serial + ", " + path + ", " + timestamp + ", " + timestamps[i])
	}
