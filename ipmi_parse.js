...
function munchNewMessageStart(lines, i, in_flight) {
  ...
  let entry = { netfn: 0, lun: 0, cmd: 0, serial: -999,
		            start_usec: 0, end_usec: 0, request: [], response: [] }
	entry.start_usec = ts
	entry.serial     = serial
  ...
  x = munchArrayOfBytes(lines, j)
	if (x[0] == null) { entry.request = [] } // Truncated
	entry.request = x[0]; j = x[1]
...
function munchNewMessageEnd(lines, i, in_flight, parsed_entries) {
  ...
  let x = munchByte(lines, i+2) // Skip "struct {"
	if (x[0] == null) { return [null, i] } // NetFN
	if (entry.netfn + 1 != x[0]) { return [null, i ] }
	let j = x[1]
	...
	x = munchByte(lines, j) // CMD
  if (x[0] == null) { return [null, i] }
	if (entry.cmd != x[0]) { return [null, i] }
  j = x[1]
...
function MunchLines() {
    ...
    if (line.indexOf("interface=org.openbmc.HostIpmi") != -1 &&
        line.indexOf("member=ReceivedMessage") != -1) {
			...
			i = x[1];
			if (entry != null) { g_ipmi_in_flight[entry.serial] = entry }
    } else if (line.indexOf("interface=org.openbmc.HostIpmi") != -1 &&
               line.indexOf("member=sendMessage") != -1) {
			let x = munchLegacyMessageEnd(g_ipmi_parse_lines, i, g_ipmi_in_flight, g_ipmi_parsed_entries);
			if (i != x[1]) lidx_last = x[1] // Munch success!
			i = x[1]
      ...
      let x = munchNewMessageEnd(g_ipmi_parse_lines, i, g_ipmi_in_flight, g_ipmi_parsed_entries)
			if (i != x[1]) lidx_last = x[1] // Munch success
			i = x[1]
    }
    i++
	...
	console.log(lidx_last + " lines munched, |lines|=" + g_ipmi_parse_lines.length + ", |entries|=" + g_ipmi_parsed_entries.length, ", |inflight|=" + Object.keys(g_ipmi_in_flight).length)
}
