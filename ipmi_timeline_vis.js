...
function ComputeHistogram(num_buckets = 30, is_free_x = true) {
  let global_lb = Infinity, global_ub = -Infinity
	...
	for (let i=0; i<Intervals.length; i++) {
		...
		let l = Math.min.apply(Math, interval.map(function(x) { return x[1] - x[0] }))
		let u = Math.max.apply(Math, interval.map(function(x) { return x[1] - x[0] }))
  ...
  HistoryHistogram = []
	for (let i=0; i<Intervals.length; i++) {
	  ...
	  if (is_free_x == true) {
		  lb = Math.min.apply(Math, interval.map(function(x) { return x[1] - x[0] }))
			ub = Math.max.apply(Math, interval.map(function(x) { return x[1] - x[0] }))
		}
		const EPS = 1e-2;
		if (lb == ub) ub = lb + EPS;
...
function GenerateTimeLine(grouped) {
  ...
  for (let i=0; i<sortedKeys.length; i++) {
    ...
    for (let j=0; j<grouped[sortedKeys[i]].length; j++) {
      ...
      line.push([parseFloat(entry[2]) / 1000000,
                 parseFloat(entry[3]) / 1000000, entry]);
    }
    Intervals.push(line);
  }
}
...
function MapXCoord(x, left_margin, right_margin, rl, rr) {
  ...
  if (ret < left_margin) { ret = left_margin; }
  else if (ret > right_margin) { ret = right_margin; }
  return ret;
}
...
function UpdateAnimation() {
  ...
  if (IsAnimating) {
    let t = 0.80;
    LowerBoundTime = LowerBoundTime       * t +
                     LowerBoundTimeTarget * (1-t);
    UpperBoundTime = UpperBoundTime       * t +
                     UpperBoundTimeTarget * (1-t);
    IsCanvasDirty = true;
  }
}
...
function RenderHistogramForImageData(ctx, key) {
  ...
  for (let i=0; i<buckets.length; i++) {
    if (maxCount < buckets[i]) { maxCount = buckets[i]; }
  ...
  for (let i=0; i<buckets.length; i++) {
    ...
    if (buckets[i] > 0) {
      let dx0 = x + PAD2 + (w - 2*PAD2) * 1.0 * i / buckets.length,
          ...
          dy0 = y + h - h * 1.0 * buckets[i] / maxCount,
          dy1 = y + h;
      ...
      if (cumDensity0 <  1-HISTOGRAM_RIGHT_TAIL_WIDTH &&
        ...
        threshEntry[1][1] = hist[0] + (hist[1] - hist[0]) /bucketsLen * (i-1);
      }
...
function RenderHistogram(ctx, key, xMid, yMid) {
  if (GetHistoryHistogram()[key] == undefined) { return; }
  if (IpmiVizHistogramImageData[key]  == undefined) { return; }
  let hist = GetHistoryHistogram()[key];
  ctx.putImageData(IpmiVizHistogramImageData[key],
                   xMid - HISTOGRAM_W/2,
                   yMid - HISTOGRAM_H/2);
...
function draw_timeline(ctx) {
  ...
  if (IsCanvasDirty) {
    ...
    ctx.textAlign = "left";
    ctx.fillText("" + LowerBoundTime.toFixed(toFixedPrecision) + "s",
                 LEFT_MARGIN + 3, height);
    ctx.textAlign = "end";
    ctx.fillText("" + UpperBoundTime.toFixed(toFixedPrecision) + "s",
    ...
    if (GroupByStr.length > 0) { columnTitle = GroupByStr; }
    ctx.textAlign = "right";
    ctx.textBaseline = "top";
    ...
    // Time Axis Breaks
    const breakWidths = [3600, 60, 30, 10, 5, 2, 1, 0.5, 0.2, 0.1,
      ...
      if (bidx < breakWidths.length) {
        for (; t2 < UpperBoundTime; t2 += breakWidth) {
          if (t2 > LowerBoundTime) {
            ctx.beginPath();
            let dx = MapXCoord(t2, LEFT_MARGIN, RIGHT_MARGIN,
    ...
    for (let j=0; j<Intervals.length; j++) {
      ...
      for (let i=0; i<Intervals[j].length; i++) {
        ...
        if (ub < LowerBoundTime) { numOverflowEntriesToTheLeft  ++; continue; }
        if (lb > UpperBoundTime) { numOverflowEntriesToTheRight ++; continue; }
      ...
      // Triangle markers for entries outside of the viewport
      {
        ...
        if (numOverflowEntriesToTheLeft > 0) {
          ...
          ctx.fillText("+" + numOverflowEntriesToTheLeft,
        ...
        if (numOverflowEntriesToTheRight > 0) {
          ...
          ctx.fill();
          ctx.textAlign = "right";
          ctx.fillText("+" + numOverflowEntriesToTheRight,
    ...
    if (IsHighlighted()) {
      ctx.fillStyle = "rgba(128,128,255,0.5)";
      let x0 = MapXCoord(t0, LEFT_MARGIN, RIGHT_MARGIN,
                         LowerBoundTime, UpperBoundTime);
      let x1 = MapXCoord(t1, LEFT_MARGIN, RIGHT_MARGIN,
                         LowerBoundTime, UpperBoundTime);
...
function MouseXToTimestamp(x) {
  let ret = (x - LEFT_MARGIN) / (RIGHT_MARGIN - LEFT_MARGIN) *
            (UpperBoundTime - LowerBoundTime) + LowerBoundTime;
...
function OnMouseMove() {
  const PAD = 2;
  if (MouseState.x < LEFT_MARGIN) MouseState.hovered = false;
  else if (MouseState.x > RIGHT_MARGIN) MouseState.hovered = false;
  else MouseState.hovered = true;
  IsCanvasDirty = true;
  ...
  if (lineIndex < Intervals.length) {
    ...
    if (MouseState.x <= PAD + LINE_SPACING/2 + LEFT_MARGIN &&
      ...
      MouseState.hoveredSide = "left";
      IsCanvasDirty = true;
    } else if (MouseState.x <= RIGHT_MARGIN - PAD &&
...
function GenerateIPMIToolIndividualCommandReplay(reqs) {
  ...
  for (let i=0; i<reqs.length; i++) {
    ...
    x = x + "ipmitool raw " + req[0] + " "
          + req[1] + " " + ToHexString(req[4], "0x", " ") + "\n";
  }
...
function GenerateBusctlReplayLegacyInterface(reqs) {
  ...
  for (let i=0; i<reqs.length; i++) {
    let req = reqs[i];
    x = x + "busctl --system emit  /org/openbmc/HostIpmi/1 org.openbmc.HostIpmi ReceivedMessage yyyyay ";
    x = x + serial + " " + req[0] + " 0 " + req[1] + " "
          + req[4].length + " " + ToHexString(req[4], "0x", " ")
          + "\n";
    serial = (serial + 1) % 256;
  }
...
function GenerateBusctlReplayNewInterface(reqs) {
  ...
  for (let i=0; i<reqs.length; i++) {
    let req = reqs[i];
    x = x + "busctl --system call xyz.openbmc_project.Ipmi.Host /xyz/openbmc_project/Ipmi xyz.openbmc_project.Ipmi.Server execute yyyaya{sv} ";
