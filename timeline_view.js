...
class TimelineView {
  constructor() {
	...
	Zoom(dz, mid = undefined, iter=1) {
		...
		if (dz != 0) {
			if (mid == undefined) { mid = (this.LowerBoundTime +
				                             this.UpperBoundTime) / 2; }
	...
	BeginPanScreenAnimaton(delta_screens, iter=1) {
		...
		this.BeginSetBoundaryAnimation(this.LowerBoundTime + deltat,
	...
	UpdateAnimation() {
		...
		if (this.IsAnimating) {
			let t = 0.80;
			this.LowerBoundTime = this.LowerBoundTime       * t +
														this.LowerBoundTimeTarget * (1-t);
			this.UpperBoundTime = this.UpperBoundTime       * t +
													  this.UpperBoundTimeTarget * (1-t);
			this.IsCanvasDirty = true;
		}
	}

	IsHighlighted() {
		return (this.HighlightedRegion.t0 != -999 &&
			      this.HighlightedRegion.t1 != -999);
	}

	RenderHistogram(ctx, key, xMid, yMid) {
		if (GetHistoryHistogram()[key] == undefined) { return; }
		if (this.IpmiVizHistogramImageData[key]  == undefined) { return; }
		let hist = GetHistoryHistogram()[key];
		ctx.putImageData(this.IpmiVizHistogramImageData[key],
										 xMid - HISTOGRAM_W/2,
										 yMid - HISTOGRAM_H/2);
		...
		ctx.textAlign = "left";
		ctx.fillStyle = "#000";
		if (hist[1] > 1000) { ub = (hist[1]/1000.0).toFixed(1) + "ms";
		} else { ub = hist[1].toFixed(1) + "us"; }
  ...
  MouseXToTimestamp(x) {
		let ret = (x - LEFT_MARGIN) / (RIGHT_MARGIN - LEFT_MARGIN) *
							(this.UpperBoundTime - this.LowerBoundTime) + this.LowerBoundTime;
    ...
    // Update highlighted area
		...
		const PAD = 2;
		if      (this.MouseState.x < LEFT_MARGIN) this.MouseState.hovered = false;
		else if (this.MouseState.x > RIGHT_MARGIN) this.MouseState.hovered = false;
		else     this.MouseState.hovered = true;

		this.IsCanvasDirty = true;
		let lineIndex = Math.floor((this.MouseState.y - YBEGIN + TEXT_Y0) / LINE_SPACING);
		...
		if (lineIndex < this.Intervals.length) {
			...
			if (this.MouseState.x <= PAD + LINE_SPACING/2 + LEFT_MARGIN &&
				...
				this.IsCanvasDirty = true;
			} else if (this.MouseState.x <= RIGHT_MARGIN - PAD &&
      ...
      if (this.MouseState.hoveredSide == "left") {
        ...
        for (let i=0; i<line.length; i++) {
          if (line[i][0] >= this.UpperBoundTime) {
						// TODO: pass timeline X to linked view
            this.BeginWarpToRequestAnimation(line[i]); break;
          }
        }
      }
    }
	...
	ScrollY(delta) {
		...
		if (this.TitleStartIdx < 0) { this.TitleStartIdx = 0; }
	...
	Render(ctx) {
		...
		{
			if (extent < 0.1) { toFixedPrecision = 4;
			} else if (extent < 1) { toFixedPrecision = 3; }
		}
    ...
    let theHoveredReq       = undefined;
      ...
      this.IsCanvasDirty = false;
			...
			let numIntersected = 0; // How many requests intersect with highlighted area
			...
			ctx.textAlign = "left";
			ctx.fillText("" + this.LowerBoundTime.toFixed(toFixedPrecision) + "s",
									 LEFT_MARGIN + 3, height);
			ctx.textAlign = "end";
			ctx.fillText("" + this.UpperBoundTime.toFixed(toFixedPrecision) + "s",
			...
			if (this.GroupByStr.length > 0) { columnTitle = this.GroupByStr; }
			ctx.textAlign = "right";
			ctx.textBaseline = "top";
			// Split into lines
			{
			...
			// Time Axis Breaks
			const breakWidths = [86400, 10800, 3600, 60, 30, 10, 5, 2, 1, 0.5, 0.2, 0.1,
			...
			if (this.MouseState.hovered     == true &&
				...
				if (this.IsHighlighted() == false) {
					...
					let label = "" +
							this.MouseXToTimestamp(this.MouseState.x).toFixed(toFixedPrecision) + "s";
